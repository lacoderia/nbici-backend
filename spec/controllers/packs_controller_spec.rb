feature 'PacksController' do
  ...
  context 'Get packs normally' do
    ...
    let!(:special_price_off_start_date) { create(:configuration, key: "special_prices_start_date", value: Time.zone.now - 2.hour ) }
    let!(:special_price_off_end_date) { create(:configuration, key: "special_prices_end_date", value: Time.zone.now - 1.hour ) }
  ...
  context 'Get packs forcing special prices' do
    ...
    let!(:special_price_on_start_date) { create(:configuration, key: "special_prices_start_date", value: Time.zone.now ) }
    let!(:special_price_on_end_date) { create(:configuration, key: "special_prices_end_date", value: Time.zone.now + 1.hour ) }
